<template>
  <div class="mb-1">
    <component
      :is="collapsible ? 'button' : 'router-link'"
      :to="collapsible ? undefined : to"
      @click="collapsible && toggle()"
      class="w-full flex items-center justify-between px-3 py-2.5 text-slate-700 dark:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg group transition"
    >
      <div class="flex items-center gap-3">
        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" v-html="iconPath"></svg>
        <span class="font-medium text-sm">{{ label }}</span>
      </div>

      <div class="flex items-center gap-2">
        <span v-if="badge" class="px-2 py-0.5 text-xs font-semibold text-green-700 bg-green-100 rounded-full">{{ badge }}</span>
        <svg v-if="collapsible" :class="['w-4 h-4 transition-transform', isOpen ? 'transform rotate-180' : '']" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const iconPath = computed(() => {
  ...
  return icons[props.icon] || icons['document-text'];
});
</script>
