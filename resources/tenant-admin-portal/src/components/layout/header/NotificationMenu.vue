<template>
  <div class="relative" ref="dropdownRef">
    ...
    >
      <span
        v-if="notifying"
        class="absolute -right-1 -top-1 z-1 flex h-4 w-4 items-center justify-center rounded-full bg-brand-500 text-[10px] font-medium text-white"
      >
          ...
          >
            <span class="block">
              <span class="mb-1.5 block text-theme-sm text-gray-800 dark:text-white/90">
                <span class="font-semibold">{{ notification.title }}</span>: {{ notification.body }}
              </span>

              <span class="flex items-center gap-2 text-gray-500 text-theme-xs dark:text-gray-400">
                <span>{{ notification.timestamp }}</span>
                <span v-if="!notification.read" class="w-2 h-2 bg-brand-500 rounded-full"></span>
              </span>
            </span>
          </div>
        </li>
